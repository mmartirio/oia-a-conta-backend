package com.oiaaconta.whatsapp.util;
...
// os magic bytes reais do conteúdo decodificado, não só um prefixo de texto.
public final class ImagemValidator {
    ...
    public static void validar(String dataUri) {
        ...
        if (!m.matches()) {
            throw new IllegalArgumentException("Arquivo inválido. Envie uma imagem (PNG, JPG ou WEBP).");
        }
        String tipo = m.group(1);
        if (!tipo.equals("image/jpeg") && !tipo.equals("image/png") && !tipo.equals("image/webp")) {
            throw new IllegalArgumentException("Formato de imagem não suportado. Envie PNG, JPG ou WEBP.");
        }
